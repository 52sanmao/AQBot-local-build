...
import { Brain, ChevronDown, ChevronRight, AlertCircle, BookOpen, Database } from 'lucide-react';
...
export function MemoryRetrievalNode(props: NodeComponentProps<MemoryRetrievalNodeData>) {
    ...
    return (
      ...
      >
        ...
        <span style={{ color: token.colorTextSecondary, fontSize: 13 }}>
          {t('chat.memory.searching', '正在检索记忆...')}
        </span>
      </div>
    );
  }
  ...
  if (status === 'error') {
    return (
      ...
      >
        <AlertCircle size={16} />
        <span>{node.content || t('chat.memory.error', '记忆检索失败')}</span>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      {/* Source summary pills (always visible) */}
      <div
        style={{
          display: 'flex',
          gap: 6,
      ...
      >
        {sources.map((src, i) => {
          const Icon = src.source_type === 'knowledge' ? BookOpen : Database;
          const label = src.source_type === 'knowledge'
            ? t('chat.memory.knowledgeBase', '知识库')
            : t('chat.memory.memoryNamespace', '记忆');
          return (
            <span
              key={i}
              style={{
